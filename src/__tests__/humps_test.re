...
describe("camelize", () => {
  test("converts underscored strings to camelcase.", () =>
  ...
  test("converts PascalCased strings to camelcase.", () =>
...
describe("decamelize", () => {
  test("converts camelcased strings to underscored.", () =>
    expect @@ decamelize("helloWorld") |> toBe("hello_world")
  );
  test("does not separate on digits.", () =>
    expect @@ decamelize("helloWorld1") |> toBe("hello_world1")
  );
});

describe("decamelizeWithOptions", () => {
  test("decamelizes strings with custom separator.", () =>
    expect @@
    decamelizeWithOptions("helloWorld") @@
    makeOptions(~separator="-", ())
    |> toBe("hello-world")
  );
  test("uses a custom split regexp.", () =>
    expect @@
    decamelizeWithOptions("helloWorld1") @@
...
describe("pascalize", () => {
  test("converts underscored strings to PascalCase.", () =>
...
describe("depascalizeWithOptions", () => {
  test("decamelizes strings with custom separator.", () =>
    expect @@
    depascalizeWithOptions("HelloWorld") @@
    makeOptions(~separator="-", ())
    |> toBe("hello-world")
  );
  test("uses a custom split regexp.", () =>
    expect @@
    depascalizeWithOptions("HelloWorld1") @@
    makeOptions(~split=[%re "/(?=[A-Z0-9])/"], ())
    |> toBe("hello_world_1")
  );
});
